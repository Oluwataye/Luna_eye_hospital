...
export const Results: React.FC = () => {
  ...
  const handleSubmitResult = async (e: React.FormEvent) => {
    ...
    try {
      await api.updateInvestigationResult(selectedInv.id, { results_notes: resultNotes, status: 'Completed' });
                ...
                investigations.filter(inv => 
                  ...
                  >
                    ...
                    <div style={{ width: '100%', marginTop: '12px', paddingTop: '12px', borderTop: '1px solid rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <span className="leh-label" style={{ fontSize: '10px' }}>ID: {inv.patient_id.slice(0, 8)}</span>
                    </div>
                  </button>
                ))
              )}
