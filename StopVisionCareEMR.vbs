...
' 1. Find the Process ID (PID) listening on Port 80 and kill it
' This specifically targets our application server without affecting other node processes
WshShell.Run "cmd /c for /f ""tokens=5"" %a in ('netstat -ano ^| findstr :80 ^| findstr LISTENING') do taskkill /f /pid %a", 0, True
